...
export const AuditFeedSection = ({
  ...
  return (
    <>
      <form className="filters" onSubmit={(e) => { e.preventDefault(); onApply() }}>
        <label>Event type<input value={filters.auditType} onChange={(e) => patch('auditType', e.target.value)} placeholder="item_moderated" /></label>
        <label>Actor<input value={filters.auditActor} onChange={(e) => patch('auditActor', e.target.value)} placeholder="Telegram ID" /></label>
        <label>Item<input value={filters.auditItem} onChange={(e) => patch('auditItem', e.target.value)} placeholder="Item ID" /></label>
        <label>Claim<input value={filters.auditClaim} onChange={(e) => patch('auditClaim', e.target.value)} placeholder="Claim ID" /></label>
        <label>From<input type="datetime-local" value={filters.auditCreatedFrom} onChange={(e) => patch('auditCreatedFrom', e.target.value)} /></label>
        <label>To<input type="datetime-local" value={filters.auditCreatedTo} onChange={(e) => patch('auditCreatedTo', e.target.value)} /></label>
        <label>Limit<select value={filters.auditLimit} onChange={(e) => patch('auditLimit', Number(e.target.value))}><option value={20}>20</option><option value={30}>30</option><option value={50}>50</option></select></label>
